...
class drivetrain:
    ...
    def control_motor(left_forward: int, left_back: int, right_forward: int, right_back: int):
        left_forward_wheel.set_power(left_forward * 0.9)  # LEFT FORWARD
        right_back_wheel.set_power(-right_back)  # RIGHT BACK  
        right_forward_wheel.set_power(-(right_forward))      # RIGHT FORWARD
        left_back_wheel.set_power(left_back * 0.9)             # LEFT BACK
...
class automatic_stage:
    ...
    def right_auto():
        # Stage 1
        led.show("auto", wait=False)
        drivetrain.control_motor(50, -50, -50, 50)
        time.sleep(1)
        drivetrain.control_motor(-50, -50, -50, -50)
        time.sleep(0.6)
        drivetrain.stop()
        drivetrain.control_motor(35, 35, 35, 35)
        time.sleep(0.2)
        drivetrain.control_motor(0,0,0,0)
        while back_ranging.get_distance() < 105:
            led.show(back_ranging.get_distance(), wait=False)
            # drivetrain.set_velocity(100)
            drivetrain.control_motor(35, 35, 35, 35)
        drivetrain.stop()
        time.sleep(2)
        led.show("S0", wait=False)
        drivetrain.set_velocity(35)
        while back_ranging.get_distance() > 14:        
            led.show(back_ranging.get_distance(), wait=False)
            drivetrain.turn(LEFT)
        drivetrain.stop()
        drivetrain.control_motor(-50, -50, -50, -50)
        ...
        drivetrain.stop()
        time.sleep(2)
        led.show("S1", wait=False)
        while back_ranging.get_distance() < 80:
            led.show(back_ranging.get_distance(), wait=False)
